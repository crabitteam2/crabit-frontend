...
it.each([
  ["coin", CoinPage],
  ["done", DonePage],
] as const)("rejects missing or invalid deposit sources at the %s boundary", async (_, Page) => {
...
it.each([
  ["coin", CoinPage],
  ["done", DonePage],
] as const)("accepts the preserved source at the %s boundary", async (_, Page) => {
  const page = await Page({
    params: Promise.resolve({ wishId: "w4" }),
