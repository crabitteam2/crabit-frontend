...
describe("/api/demo/persona", () => {
  ...
  it.each(PERSONAS)("sets only the Demo namespace cookie for %s", async (persona) => {
    const tokens = configureDemo("local");
    const response = await POST(new Request("http://frontend.test/api/demo/persona", {
      method: "POST",
    ...
    expect(response.headers.get("set-cookie")).not.toContain("crabit-e2e-persona");
  ...
  it("clears only the Demo namespace cookie", async () => {
    ...
    const response = await DELETE(new Request(
      "https://frontend.test/api/demo/persona",
      { method: "DELETE" },
    ));
...
function configureDemo(appEnv: "local" | "prod") {
  ...
  vi.stubEnv("BACKEND_URL", appEnv === "prod"
    ? "https://backend.test"
    : "http://backend.test");
