  ...
  return (
    <div className="flex min-h-svh flex-col">
      <header className="border-gray-3 flex items-center justify-between border-b px-4 pt-[calc(env(safe-area-inset-top)+12px)] pb-4">
        ...
        <p className="text-fg-neutral flex items-center gap-1 text-[15px] leading-5 font-medium tracking-[-0.3px]">
          {year}
      ...
      <nav
        aria-label="월 선택"
        className="flex items-center justify-center gap-12 px-4 py-5"
      >
        {months.map((month) =>
          month.href === null ? (
            <span
              key={month.label}
              aria-current="page"
              className="text-fg-neutral text-[20px] leading-7 font-medium tracking-[-0.3px]"
            >
              {month.label}
            </span>
          ) : (
            <Link
              key={month.label}
              href={month.href}
              className="text-gray-5 text-[20px] leading-7 font-medium tracking-[-0.3px]"
            >
              {month.label}
            </Link>
          ),
      ...
      <p className="text-fg-neutral-muted px-4 py-5 text-center text-[20px] leading-7 font-medium tracking-[-0.3px]">
        {message}
      </p>
    </div>
  );
