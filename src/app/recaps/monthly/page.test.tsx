...
vi.mock("@/lib/persona/display-name-server", () => ({ readPersonaDisplayName: async () => state.name }));
vi.mock("../load-recap", () => ({ loadMonthlyRecap: async () => ({
  cardBalanceAccountId: "selected-account",
  recap: { status: state.status, period: { startDate: "2026-08-01" }, result: state.status === "SUCCEEDED" ? {
    typeSection: { typeTitle: "꾸준형", message: "실제 응답 문구" },
    objectivePerformance: {}, patternAnalysis: {}, groupComparison: {}, pacePrediction: {},
  } : null },
}) }));
vi.mock("../_components/monthly-recap-screen", () => ({ MonthlyRecapScreen: ({ intro }: { intro: string }) => <p>{intro}</p> }));
vi.mock("../_components/monthly-recap-empty", () => ({ MonthlyRecapEmpty: ({ message }: { message: string }) => <p>{message}</p> }));
...
it("renders the selected representative and updates it after a switch", async () => {
  ...
  for (const grade of [4, 5]) {
    ...
    render(await MonthlyRecapPage({ searchParams: Promise.resolve({ month: "2026-08" }) }));
