export interface paths {
    ...
    "/v1/card-balance-accounts/{cardBalanceAccountId}/fund-movements": {
         ...
         * @description 외부 카드 잔액 변경과 모든 위시 자금 이동을 포함해 불변 원장 이벤트마다 항목 하나를 반환합니다. 위시 이체는 두 위시 효과를 갖지만 계정 이력에서는 항목 하나입니다. 응답 필드와 일곱 이벤트 타입은 유지합니다. 보정은 새 보상 이벤트로 기록하며 이전 이벤트를 수정하거나 삭제하지 않습니다.
         *     기간은 occurredAt 기준 from 포함, to 미포함입니다. 각 경계는 Z 또는 명시적 UTC 오프셋이 있는 RFC 3339 타임스탬프이며 생략한 쪽은 무제한입니다. 빈 값·잘못된 형식·시간대 누락 또는 from >= to는 400 MALFORMED_REQUEST입니다. 같은 시점을 나타내는 오프셋은 같은 UTC 조건으로 정규화하며 저장된 시간 정밀도를 보존합니다. API 기본값은 전체 기간입니다. 홈 화면의 최근 3개월은 클라이언트가 Asia/Seoul 달력 기준 경계를 명시하며 x-client-contract를 따릅니다.
         *     q는 페이지를 자르기 전 선택 기간 전체에서 검색합니다. 전달값의 정규화 전 Unicode 코드 포인트가 100개를 넘으면 400입니다. 앞뒤 공백을 제거하고 연속 공백을 한 칸으로 통합하며 검색 대상에도 같은 공백 정규화를 적용합니다. 빈 검색어는 생략과 같습니다. Latin 대소문자를 일관되게 접어 정규화한 전체 검색어로 부분 일치하며 AND 토큰 검색이 아닙니다. ledger_wish_effect.wish_purpose_snapshot의 역사적 위시 이름(삭제·이름 변경 포함), 아래 한국어 이벤트 이름 또는 절대 원화 금액 정확 일치 중 하나면 포함합니다. 현재 위시 이름으로 스냅샷을 대체하지 않습니다. 이체는 source/destination 어느 이름으로도 검색하지만 한 번 반환합니다. 숫자 검색어도 이름 검색에 참여합니다. %, _, 역슬래시 등 SQL 패턴 문자는 리터럴이며 SQL 값 바인딩과 LIKE 이스케이프를 적용합니다.
         *     종류 검색 이름은 CARD_BALANCE_CHANGE=카드 잔액 변경, WISH_DEPOSIT=위시 넣기, WISH_WITHDRAWAL=위시 빼기, WISH_TRANSFER=위시 간 이동, WISH_COMPLETION_RETURN=위시 완료 반환, WISH_ABANDONMENT_RETURN=위시 포기 반환, WISH_DELETION_RETURN=위시 삭제 반환입니다. 금액은 선택적인 선행 +/- 부호, ASCII 정수 또는 올바른 천 단위 쉼표, 선택적인 마지막 원(앞 공백 허용)을 받습니다. 부호와 선행 0은 무시합니다. 5000, 5,000, +5,000원, -5,000 원은 절대 5000원과 같습니다. 0..9007199254740991 밖 값, 소수, 잘못된 쉼표는 금액 조건에는 일치하지 않지만 일반 텍스트 검색으로 남으며 요청 오류가 아닙니다. 카드 변경은 abs(actualCardBalanceDelta), 이체는 양수 amount 한 번, 그 외는 abs(accountAvailableBalanceDelta)를 비교합니다.
         *     sort 기본값 desc는 occurredAt DESC, eventId DESC이고 asc는 occurredAt ASC, eventId ASC입니다. UUID 비교는 DB UUID 순서입니다. 빈 값·대문자·알 수 없는 sort는 400입니다. 정렬 버전과 같은 타임스탬프의 eventId 순서를 커서와 SQL에서 동일하게 사용합니다. desc는 마지막 튜플보다 작은 값, asc는 큰 값으로 엄격히 뒤에 이어지며 limit+1개의 일치 이벤트를 선택합니다. 다른 일치 항목이 있을 때만 nextCursor를 발급하며 빈 페이지와 마지막 페이지는 null입니다.
         *     새 첫 페이지는 v2 불투명 URL-safe 커서를 발급합니다. 작업, 정확한 계정 ID, 커서/정렬 버전, 정규화한 from/to/q/sort, 마지막 (occurredAt,eventId), 첫 조회 시 그 계정에 커밋된 최대 application_order 상한을 묶습니다. 모든 페이지는 application_order <= 상한으로 제한하여 이후 추가된 일반·과거 시각·같은 타임스탬프 이벤트를 제외합니다. 새 이벤트는 커서를 버리고 새 첫 페이지를 조회해야 포함됩니다. 유효한 커서에는 유효한 limit 값을 함께 사용할 수 있습니다. limit는 바인딩에서 제외하므로 페이지마다 바꿀 수 있습니다.
         *     다음 요청은 동일한 실효 정규화 조건을 반복해야 합니다. 생략값은 기본값으로 해석하며 커서에서 필터를 자동 복구하지 않습니다. 버전·필드 개수와 타입·작업/계정/조건·튜플·음수 아닌 상한을 검증합니다. 빈 값, 잘못된 형식, 미지원 버전 또는 바인딩이 일치하지 않는 커서는 부분 페이지 없이 400 MALFORMED_REQUEST입니다. ErrorEnvelope.fieldErrors는 잘못된 매개변수를 가리키며 기간 역전은 to를 가리킬 수 있습니다. 커서는 조회 위치일 뿐 소유권 증거가 아니며 새 서명 키나 인증 정책을 도입하지 않습니다.
         *     기존 v1 계정 커서는 from/to 생략, q 빈 값, sort desc의 기본 조건에서만 허용하고 기존 튜플 순회를 유지하며 연속 페이지도 v1을 발급합니다. v1에는 첫 페이지 상한 보장이 없으며 다른 조건에서는 400입니다. 카드 잔액 변경 및 위시별 이력의 v1 동작은 유지합니다.
         *     기간·검색·커서·application_order 상한은 페이지 이벤트 키 선택에만 적용합니다. EVENT_FACTS 역사적 잔액 재구성은 현재 잔액을 되돌리는 데 필요한 이후 이벤트를 포함한 전체 계정 이벤트를 사용하므로 같은 이벤트의 accountAvailableBalanceAfter는 필터가 바뀌어도 같습니다. 상한은 이벤트 집합만 제한합니다. 권한과 소유권은 요청할 때마다 다시 평가하고 현재 위시 삭제 표시도 재평가하며 캐시 가능성을 보장하지 않습니다. 기존 학생 역할, 학원, 활성 계정과 소유권 정책 및 접근 불가 계정을 숨기는 404를 보존합니다. 인증은 SyntheticBearer, 401은 WWW-Authenticate: Bearer를 유지합니다. 새로운 토큰·쿠키·인증 주체·멱등 정책은 없습니다.
         */
...
export interface components {
    schemas: {
        ...
        AccountFundMovementPage: {
            /** @description 선택 기간과 검색에 일치하는 외부 카드 변경 및 모든 위시 이동의 불변 원장 이벤트당 하나의 항목입니다. sort=desc(기본)는 occurredAt DESC, eventId DESC, sort=asc는 occurredAt ASC, eventId ASC이며 UUID는 DB 순서를 따릅니다. 이체는 한 항목이며 역사적 잔액은 전체 계정 이벤트로 재구성합니다. */
            items: components["schemas"]["AccountFundMovement"][];
            /** @description 다른 일치 항목이 있을 때 마지막 (occurredAt,eventId) 튜플에서 파생된 불투명 커서입니다. v2는 작업·계정·정규화 from/to/q/sort·첫 페이지 application_order 상한을 묶으며 동일 조건을 반복해야 합니다. limit는 변경 가능합니다. 기본 조건의 기존 v1 연속 체인은 v1을 유지합니다. 빈 페이지와 마지막 페이지는 null입니다. */
    ...
    parameters: {
        AcademyId: components["schemas"]["Uuid"];
        /**
         * @description occurredAt의 포함 하한입니다. Z 또는 명시적 UTC 오프셋이 있는 RFC 3339 타임스탬프를 사용합니다. 생략하면 하한이 없으며 빈 값·잘못된 형식·시간대 누락은 400입니다.
         * @example 2026-06-16T15:00:00Z
         */
        AccountFundMovementFrom: string;
        /**
         * @description 페이지 분할 전에 선택 기간 전체를 검색합니다. 전달값의 정규화 전 최대 길이는 Unicode 코드 포인트 100개이며 초과하면 400입니다. 앞뒤 공백을 제거하고 연속 공백을 한 칸으로 합치며 빈 결과는 생략과 같습니다. 역사적 위시 이름 또는 한국어 종류 이름의 대소문자 무시 부분 일치, 또는 절대 원화 금액의 정확 일치로 검색합니다. 세부 검색·금액 정규화는 작업 설명을 따릅니다.
         * @example 여름 캠프
         */
        AccountFundMovementQuery: string;
        /**
         * @description occurredAt과 eventId를 함께 정렬합니다. desc는 두 값 모두 내림차순, asc는 모두 오름차순이며 UUID는 DB UUID 순서를 따릅니다. 기본값은 desc이고 빈 값·대문자·알 수 없는 값은 400입니다.
         * @example desc
         */
        AccountFundMovementSort: "desc" | "asc";
        /**
         * @description occurredAt의 미포함 상한입니다. Z 또는 명시적 UTC 오프셋이 있는 RFC 3339 타임스탬프를 사용합니다. 생략하면 상한이 없습니다. from과 함께 지정하면 from < to여야 하며 빈 값·잘못된 형식·시간대 누락·기간 역전은 400입니다.
         * @example 2026-09-17T15:00:00Z
         */
        AccountFundMovementTo: string;
        CardBalanceAccountId: components["schemas"]["Uuid"];
    ...
    listAccountFundMovements: {
        parameters: {
            query?: {
                 ...
                 * @example 2026-06-16T15:00:00Z
                 */
                from?: components["parameters"]["AccountFundMovementFrom"];
                limit?: components["parameters"]["Limit"];
                /**
                 * @description 페이지 분할 전에 선택 기간 전체를 검색합니다. 전달값의 정규화 전 최대 길이는 Unicode 코드 포인트 100개이며 초과하면 400입니다. 앞뒤 공백을 제거하고 연속 공백을 한 칸으로 합치며 빈 결과는 생략과 같습니다. 역사적 위시 이름 또는 한국어 종류 이름의 대소문자 무시 부분 일치, 또는 절대 원화 금액의 정확 일치로 검색합니다. 세부 검색·금액 정규화는 작업 설명을 따릅니다.
                 * @example 여름 캠프
                 */
                q?: components["parameters"]["AccountFundMovementQuery"];
                /**
                 * @description occurredAt과 eventId를 함께 정렬합니다. desc는 두 값 모두 내림차순, asc는 모두 오름차순이며 UUID는 DB UUID 순서를 따릅니다. 기본값은 desc이고 빈 값·대문자·알 수 없는 값은 400입니다.
                 * @example desc
                 */
                sort?: components["parameters"]["AccountFundMovementSort"];
                /**
                 * @description occurredAt의 미포함 상한입니다. Z 또는 명시적 UTC 오프셋이 있는 RFC 3339 타임스탬프를 사용합니다. 생략하면 상한이 없습니다. from과 함께 지정하면 from < to여야 하며 빈 값·잘못된 형식·시간대 누락·기간 역전은 400입니다.
                 * @example 2026-09-17T15:00:00Z
                 */
                to?: components["parameters"]["AccountFundMovementTo"];
            };
            header?: never;
            path: {
                cardBalanceAccountId: components["parameters"]["CardBalanceAccountId"];
