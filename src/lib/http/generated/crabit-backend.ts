export interface paths {
    ...
    "/v1/academies/{academyId}/shared-cards": {
         ...
         * @description 조회할 때마다 학원 소속, 위시 visibility, 친구 관계, 양방향 차단을 다시 평가하고 소유자 본인은 제외합니다. photoId, object key, 과거 signed URL은 이 권한 검사를 대신하지 않습니다. PRIVATE 위시는 카드를 생성하지 않습니다. 첨부 사진이 하나라도 있으면 모든 항목의 새 5분 비공개 URL을 발급한 뒤에만 전체 페이지를 반환하며 signing 실패는 부분 페이지나 거짓 null 없이 503입니다. 임시 정렬은 contentUpdatedAt DESC, sharedCardId DESC 순입니다. 현재는 정렬 매개변수를 지원하지 않습니다. 이 임시 정책에서는 콘텐츠 또는 게시 상태가 바뀔 때만 카드 순서가 달라집니다. 친구 우선순위와 임베딩 기반 추천 정렬은 향후 계약에서 정할 사항이며 이 버전에서는 사용하지 않습니다.
         */
    ...
    "/v1/academies/{academyId}/shared-cards/{cardId}": {
        ...
        /**
         * 현재 볼 수 있는 공유 카드 조회
         * @description 소유자는 자신의 카드가 현재 공개 상태라면 조회할 수 있습니다. 다른 호출자는 현재 학원 소속, 위시 visibility, 친구 관계와 양방향 차단을 매번 다시 통과해야 하며 photoId나 과거 signed URL은 권한을 부여하지 않습니다. 사진이 있으면 새 5분 비공개 URL 세 개를 모두 발급한 뒤 반환하고 signing 실패는 거짓 null 없이 503입니다. 이미 발급된 URL은 접근이 철회되어도 최대 기존 5분 만료까지만 유효할 수 있고 새 URL은 발급하지 않습니다. 그 밖의 리소스 부재나 공개 범위 조건 위반은 모두 숨깁니다.
         */
    ...
    "/v1/card-balance-accounts/{cardBalanceAccountId}/wishes/{wishId}": {
        ...
        /**
         * 위시 논리 삭제
         * @description 최종 변경 결과를 반환합니다. 첨부 사진은 같은 domain transaction에서 즉시 접근 불가능한 DELETE_PENDING으로 바꾸며 tombstone과 불변 이력에는 사진 identity나 URL을 남기지 않습니다. 응답 capability 발급은 commit 전에 완료합니다. 이후의 모든 조회는 WISH_NOT_FOUND로 숨깁니다. OPEN 잔액 조정 건이 있어도 삭제를 차단하지 않습니다.
         */
    ...
    "/v1/wish-photos": {
        ...
        /**
         * 위시에 첨부할 비공개 사진 업로드
         * @description 인증된 학생 소유의 Pending 사진을 동기적으로 만듭니다. 정확히 하나의 photo 파트만 허용하고 5 MiB보다 큰 바이트 스트림은 디코딩 전에 중단합니다. 실제 JPEG 1080x1080 이미지만 디코딩한 뒤 EXIF·위치 메타데이터를 제거하고 1080x1080, 720x720, 360x360 JPEG로 재인코딩합니다. adult, racy, violence가 LIKELY 또는 VERY_LIKELY이면 PHOTO_CONTENT_NOT_ALLOWED로 거부하지만 medical 또는 spoof가 POSSIBLE인 것만으로는 거부하지 않습니다. 검증, 필수 콘텐츠 안전성 검사, 비공개 변형 세 개 저장, Pending 영속화가 모두 끝나기 전에는 photoId를 반환하지 않으며 부분 레코드와 객체는 보상 정리합니다. 성공한 Pending 사진은 처리가 끝난 뒤 24시간 동안만 첨부할 수 있습니다. 학생별 미첨부 Pending 사진은 최대 3개이고 rolling 1시간의 새 처리 시도는 최대 20회입니다. 유효하지 않거나 콘텐츠가 거부된 새 업로드도 시도를 소비하지만 멱등 재생은 새 시도를 소비하지 않습니다. 동일 학생의 같은 Idempotency-Key와 같은 콘텐츠인 종결 성공은 변환·검사·저장을 반복하지 않고 같은 사진 identity와 새 5분 signed URL을 반환합니다. 다른 콘텐츠로 키를 재사용하면 409이며, 일시적인 필수 의존성 실패는 종결 멱등 결과로 고정하지 않습니다. 파일 이름은 신뢰하지 않고 업로더 identity는 인증 주체에서만 결정합니다.
         */
    ...
    "/v1/wish-photos/{photoId}": {
         ...
         * @description 인증된 소유자만 미첨부 Pending 사진을 취소할 수 있습니다. 소유한 PENDING 또는 만료 사진은 즉시 첨부 불가능한 DELETE_PENDING으로 바꾸고 204를 반환하며 객체 제거와 hard delete는 비동기로 이어집니다. 같은 소유자가 DELETE_PENDING 상태에서 반복하면 204 no-op입니다. 없거나 다른 학생 소유인 식별자는 404 WISH_PHOTO_NOT_FOUND로 숨기고, 위시에 이미 첨부된 사진은 409 WISH_PHOTO_ALREADY_ATTACHED를 반환합니다. photoId나 기존 signed URL 자체는 권한을 부여하지 않습니다.
         */
...
export interface components {
    schemas: {
        ...
        ErrorEnvelope: {
            ...
            error: {
                ...
                /** @description 오류 코드별로 확장할 수 있는 메타데이터 객체입니다. 적용할 세부 정보가 없으면 비어 있으며 클라이언트는 알 수 없는 키를 무시해야 합니다. 사진 오류에는 bytes, Base64, multipart content, signed URL, object path, content digest, 콘텐츠 안전성 category·likelihood 또는 provider payload를 절대 담지 않습니다. */
                details: {
                    [key: string]: unknown;
                };
        ...
        WishPhotoUploadRequest: {
            /**
             * Format: binary
             * @description 정확히 하나만 허용되는 photo multipart 파트입니다. 선언 타입과 실제 decoded 타입이 모두 JPEG여야 하고 바이트 길이는 최대 5 MiB, decoded 크기는 정확히 1080x1080이어야 합니다. filename은 선택 사항이며 타입 판단에 신뢰하지 않습니다.
             */
            photo: string;
        };
        WishPhotoVariants: {
    ...
    responses: {
        ...
        /** @description PHOTO_PROCESSING_UNAVAILABLE — 필수 변환, 안전성 검사, 비공개 저장 또는 영속화 의존성을 일시적으로 사용할 수 없습니다. attachable identity 없이 부분 레코드와 객체를 보상 정리하며 같은 key와 콘텐츠를 재시도할 수 있습니다. */
        ...
        /** @description IDEMPOTENCY_KEY_REUSED — 같은 학생의 Idempotency-Key가 다른 업로드 콘텐츠에 이미 사용되었습니다. 변환·안전성 검사·객체 쓰기 전에 실패합니다. */
...
export interface operations {
    ...
    uploadWishPhoto: {
        ...
        responses: {
            ...
            503: components["responses"]["PhotoProcessingUnavailable"];
        };
    };
