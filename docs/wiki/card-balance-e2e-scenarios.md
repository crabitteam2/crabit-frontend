...
- Node.js 20.9 이상에서 `npm ci`를 실행한다.
...
| 값 | CLI 플래그 | 환경 변수 |
| --- | --- | --- |
| API base | `--api-base` | `E2E_API_BASE_URL` |
| 계정 UUID | `--account-id` | `E2E_CARD_BALANCE_ACCOUNT_ID` |
...
- `HTTP 401/403`: 정확한 GET/PUT/DELETE 라우트가 아니거나 백엔드가 `e2e` 프로필이 아니다. `/v1` 인증은 의도적으로 우회되지 않는다.
